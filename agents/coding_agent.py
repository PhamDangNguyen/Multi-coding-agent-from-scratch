...
class CodingAgent(BaseAgent[str, Dict[str, Any]]):
	...
	def __init__(self, llm: OpenAIClientLangChain|GeminiClientLangChain):
		self.llm = llm
		self.system_prompt_base = load_prompt("llm_systems/coding_agent.md")
...
if __name__ == "__main__":
	...
	coding_agent = CodingAgent(OpenAIClientLangChain(api_key=api_key, model=model))
	res = asyncio.run(coding_agent.run("read .txt file from llm dir", tools=BASH_TOOLS_SCHEMA))
	print(res)