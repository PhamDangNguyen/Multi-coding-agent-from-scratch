...
class SummaryAgent(BaseAgent[str, Dict[str, Any]]):
	"""Agent for summarizing task results."""

	agent_name =  "SummaryAgent"

	agent_description = "Summary Agent results task"
	def __init__(self, llm: OpenAIClientLangChain|GeminiClientLangChain):
		self.llm = llm
		self.system_prompt_base = load_prompt("llm_systems/summary_agent.md")
...
if __name__ == "__main__":
	...
	summary_agent = SummaryAgent(OpenAIClientLangChain(api_key=api_key, model=model))
	res = asyncio.run(summary_agent.run("read .txt file from llm dir", tools=BASH_TOOLS_SCHEMA))
	print(res)