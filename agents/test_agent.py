...
class TestAgent(BaseAgent[str, Dict[str, Any]]):
	...
	agent_description = "Generates and executes unit, integration, and API tests in the correct project locations. The agent detects the project’s test framework and conventions, creates mock data and automated test suites, runs the relevant tests, and reports failures with logs and root-cause hints back to the coding agent for fixing. Supports pytest, Playwright, Postman/Newman, and framework-specific test runners."
	def __init__(self, llm: OpenAIClientLangChain|GeminiClientLangChain):
		self.llm = llm
		self.system_prompt_base = load_prompt("llm_systems/testing_agent.md")
...
if __name__ == "__main__":
	...
	test_agent = TestAgent(OpenAIClientLangChain(api_key=api_key, model=model))
	res = asyncio.run(test_agent.run("read .txt file from llm dir", tools=BASH_TOOLS_SCHEMA))
	print(res)